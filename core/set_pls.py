...
class TaSetPaleoshorelines(TaBaseAlgorithm):
    ...
    def run(self):
        ...
        if topo is not None:
            self.feedback.info(
                ('Size of the Topography raster: {}'.format(topo.shape)))
        else:
            self.feedback.info(
        ...
        if vlayer.isValid() and vlayer.featureCount() > 0:
            ...
            self.feedback.error(
                "The mask layer has no features. Please add polygon features to it and try again.")
            self.kill()
        else:
            self.feedback.error(
        ...
        if self.dlg.interpolateCheckBox.isChecked():
            if not self.killed:
                ...
                self.feedback.info(
                    'In this mode the areas to emerge or submerge')
                self.feedback.info(
            ...
            if not self.killed:
                ...
                try:
                    pshoreline = polygonsToPolylines(vlayer)
            ...
            if not self.killed:
                ...
                topo_values_copied = np.empty(
                    topo.shape, dtype=taconst.NP_TopoDType)
                topo_values_copied[:] = np.nan
                topo_values_copied[(r_masks == 1) * (topo < 0)
                                   == 1] = topo[(r_masks == 1) * (topo < 0) == 1]
                topo_values_copied[(r_masks == 0) * (topo > 0)
                                   == 1] = topo[(r_masks == 0) * (topo > 0) == 1]
            ...
            if not self.killed:
                ...
                if 'topo' in locals():
                    ...
                    raster = driver.Create(
                        temp_out_file, ncols, nrows, 1, taconst.GDT_TopoDType)
                    ...
                    raster_layer = QgsRasterLayer(
                        temp_out_file, "PaleoShorelines_without_theGaps_filled", "gdal")
                    ...
                    raster_layer_ds = gdal.Open(
                        self.out_file_path, gdalconst.GA_Update)
                    topo_modified = raster_layer_ds.GetRasterBand(
                    ...
                    topo_modified[(r_masks == 0) *
                                  (topo_modified > 0)] = np.nan
                    topo_modified[(r_masks == 1) *
                                  (topo_modified < 0)] = np.nan
            ...
            if not self.killed:
                ...
                topo[(r_masks == 0) * (topo > 0) ==
                     1] = modRescale(in_array, max_depth, -0.1)
            ...
            if not self.killed:
                ...
                topo[(r_masks == 1) * (topo < 0) ==
                     1] = modRescale(in_array, 0.1, max_elev)
