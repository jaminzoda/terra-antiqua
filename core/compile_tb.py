...
class TaCompileTopoBathy(TaBaseAlgorithm):
    ...
    def getParameters(self):
        self.items = []
        self.feedback.info(
            f"{self.dlg.tableWidget.rowCount()} layers will be merged in the following order:")
        ...
        self.feedback.info(
            "Note that the overlaping data of a higher order layer will be removed.")
            ...
            if self.mask_layer and self.mask_layer.crs() != self.crs:
                try:
                    ...
                    self.feedback.warning(
                        f"Exception raised by reprojecting algorithm: {e}.")
        ...
        for name, width, height in raster_sizes:
            if current_width == width and current_height == height:
                ...
                self.feedback.error(
                    "The input raster layers have differing sizes.")
                self.feedback.error(
                    "For the layers to be properly compiled, they should be of the same size.")
                for i in raster_sizes:
                    self.feedback.error(
                        f"Layer: {i[0]}, width: {i[1]}, height: {i[2]}")
                self.kill()
                break
    ...
    def run(self):
        ...
        for i in range(len(self.items), 0, -1):
            ...
            item = self.items[i-1]
            self.feedback.info(
                f"Compiling {item.get('Layer').name()} raster layer.")
            ...
            if self.remove_overlap and item.get("Mask_Applied"):
                ...
                if self.dlg.selectedFeaturesCheckBox.isChecked():
                    ...
                    dp.addAttributes(
                        self.mask_layer.dataProvider().fields().toList())
                    temp_layer.updateFields()
                    dp.addFeatures(features)
                    dp = None
                    try:
                        buffer_layer = bufferAroundGeometries(
                            temp_layer, buffer_distance, 100, self.feedback, 10)
                    except Exception as e:
                        self.feedback.error("Something went wrong while creating buffer around polygon \
                                            geometries in the mask layer")
                        self.feedback.error(
                            "You might want to check if the mask layer contains any invalid geometry")
                        self.feedback.error(
                    ...
                    try:
                        buffer_layer = bufferAroundGeometries(
                            self.mask_layer, buffer_distance, 100, self.feedback, 10)
                    except Exception as e:
                        self.feedback.error("Something went wrong while creating buffer around polygon \
                                            geometries in the mask layer")
                        self.feedback.error(
                ...
                self.feedback.info(
                    "Removing bathymetry values from the gaps between continental blocks.")
                compiled_array[(buffer_array == 1) *
                               (compiled_array < -1000) == 1] = np.nan
                compiled_array[(masks_border_array == 1) *
                               (compiled_array < -1000) == 1] = np.nan
