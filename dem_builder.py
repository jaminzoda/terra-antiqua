...
class DEMBuilder:
        ...
        if coast_n>0:
            ...
            if deleted_n>2:
                if os.path.exists(os.path.join(out_path, "vector_masks")):
                    ...
                    print('I created a temporary folder with some shapefiles: '+os.path.join(out_path, "vector_masks"))
    ...
    def run_mask_maker(self):
        ...
        error = QgsVectorFileWriter.writeAsVectorFormat(final_masks, out_file_path, "UTF-8", masks_layer.crs(), "ESRI Shapefile")
        if error == QgsVectorFileWriter.NoError:
    ...
    def run_std_processing(self):
        ...
        if processing_type==0:
            ...
            if self.dlg5.masksFromCoastCheckBox.isChecked():
                ...
                error = QgsVectorFileWriter.writeAsVectorFormat(temp_layer, out_file, "UTF-8", mask_layer.crs(), "ESRI Shapefile")
                if error == QgsVectorFileWriter.NoError:
                    print("The  shape file" + out_file + "has been created and saved successfully")
                else:
                    print('There was some problem with writing the extracted masks on  disk.')
