...
class TopoBathyCompiler(QThread):
	...
	def run(self):
		...
		self.log.emit("Loading the vector layer with continental blocks...")
		masks_layer = self.dlg.selectMasks.currentLayer()
		if masks_layer.isValid() and masks_layer.featureCount()>0:
			self.log.emit("The layer with continental blocks is loaded properly.")
		elif masks_layer.isValid() and masks_layer.featureCount()==0:
		...
		if not self.killed:
			...
			self.log.emit("Loading the bathymetry raster layer...")
		...
		if not self.killed:
			...
			if self.dlg.selectOceanAge.currentLayer():
				self.log.emit("Loading the ocean age layer...")
				ocean_age_layer = self.dlg.selectOceanAge.currentLayer()
				if ocean_age_layer.isValid():
		...
		if not self.killed:
			...
			if self.dlg.selectSbathy.currentLayer():
				self.log.emit("Loading the shallow sea bathymetry raster layer...")
				s_bathy_layer = self.dlg.selectSbathy.currentLayer()
				if s_bathy_layer.isValid():
