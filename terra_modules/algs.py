...
class FeatureCreator(QThread):
	...
	def run(self):
		...
		self.dlg.Tabs.setCurrentIndex(1)
		self.create_closed_sea()

	def create_open_sea(self):
		input_vlayer_name = "Feature_outlines"
		input_rlayer_name = "Compiled_DEM"

		input_vlayer = QgsProject.instance().mapLayersByName(input_vlayer_name)[0]
		input_rlayer = QgsProject.instance().mapLayersByName(input_rlayer_name)[0]
		...
		input_layer_densified = processing.run("native:densifygeometriesgivenaninterval", d_params)['OUTPUT']

		# Creating random points inside feature outline polygons
		## Parameters for random points algorithm
		rp_params = {
			'INPUT': input_layer_densified,
			'STRATEGY': 1,
			'VALUE': 1,
			'MIN_DISTANCE': None,
			'OUTPUT': 'TEMPORARY_OUTPUT'
		...
		sampling_params = {
			...
			'RASTERCOPY': input_rlayer,
			'COLUMN_PREFIX': 'depth_value',
			'OUTPUT': 'TEMPORARY_OUTPUT'
		}
		points_dist_depth_layer = processing.run("qgis:rastersampling", sampling_params)['OUTPUT']

		# total = 80/points_dist_depth_layer.featureCount() if points_dist_depth_layer.featureCount() else 0
		...
		min_sea_depth = -4000
		max_shelf_depth = -100
		shelf_width = 100  # in km because distance in prevous step (distancetonearest) is in km.
		for current, feat in enumerate(features):
			...
			if dist > shelf_width + 100:  # shelf width + the width of the continental slope
		...
		input_rlayer_ds = gdal.Open(input_rlayer.source())
		topo = input_rlayer_ds.GetRasterBand(1).ReadAsArray()
		...
		pol_array = vt.vector_to_raster(
			input_layer_densified,
			input_rlayer_ds.GetGeoTransform(),
			input_rlayer.width(),
			input_rlayer.height()
		)
		...
		vlayer_line = processing.run("native:polygonstolines", ptol_params)["OUTPUT"]

		sea_boundary_array = vt.vector_to_raster(
			vlayer_line,
			input_rlayer,
			input_rlayer.width(),
			input_rlayer.height()
		...
		raster_for_interpolation.SetGeoTransform(input_rlayer_ds.GetGeoTransform())
		raster_for_interpolation.SetProjection(input_rlayer_ds.GetProjection())
	...
	# r_points_distance_layer.setName("distances")

	# QgsProject.instance().addMapLayer(r_points_distance_layer)

	def create_closed_sea(self):
