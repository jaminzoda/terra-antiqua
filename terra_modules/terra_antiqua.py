...
from .artefact_remover import PolygonCreator, ArtefactRemover, CustomFeatureSink
from .artefact_remover_dialog import ArtefactRemoverDialog
...
class TerraAntiqua:
	...
	def initGui(self):
		...
		self.add_action(
			...
			text = self.tr(u'Artefact remover'),
			callback = self.poly,
			parent = self.iface.mainWindow(),
			checkable = True)
...
# 		self.dlg6.logText.textCursor().insertHtml("{} - {} <br>".format(time, msg))
	...
	def load_artefact_remover(self):
		self.store_rubberbands_for_deleting(self.toolPoly.rubberband, self.toolPoly.vertices, self.toolPoly.points)
		self.dlg7 = ArtefactRemoverDialog()
		self.dlg7.show()
		if self.nFeatures==0:
			context = QgsExpressionContext()
			context.appendScope(QgsExpressionContextUtils.projectScope(QgsProject.instance()))
			crs = context.variable("project_crs")
			self.feature_sink = CustomFeatureSink(crs)
		
		self.dlg7.runButton.clicked.connect(self.start_artefact_remover)
		self.dlg7.cancelButton.clicked.connect(self.stop_artefact_remover)
		self.dlg7.addButton.clicked.connect(self.create_polygons_for_artefact_remover)
		self.dlg7.closeButton.clicked.connect(self.ar_clean_up)
	
	def create_polygons_for_artefact_remover(self):
		self.nFeatures+=1
		expr = self.dlg7.exprLineEdit.value()
	...
	def start_artefact_remover(self):
		expr = self.dlg7.exprLineEdit.value()
		geom = self.toolPoly.geometry
		self.feature_sink.createFeature(geom, expr)
		self.vl = self.feature_sink.getVectorLayer()
		self.dlg7.Tabs.setCurrentIndex(1)  # switch to the log tab.
		self.dlg7.cancelButton.setEnabled(True)
		self.dlg7.runButton.setEnabled(False)
		self.ar_thread = ArtefactRemover(self.vl, self.dlg7, self.iface)
		self.ar_thread.progress.connect(self.dlg7.set_progress_value)
		self.ar_thread.log.connect(self.ar_print_log)
		self.ar_thread.start()
		self.ar_thread.finished.connect(self.ar_add_result_to_canvas)
		self.nFeatures = 0
		
	
	def stop_artefact_remover(self):
		self.ar_thread.kill()
		self.dlg7.reset_progress_value()
		self.dlg7.cancelButton.setEnabled(False)
		self.dlg7.runButton.setEnabled(True)
		self.ar_print_log("The topography was NOT modified, because the user canceled processing.")
		self.ar_print_log("Or something went wrong. Please, refer to the log above for more details.")
		self.dlg7.warningLabel.setText('Error!')
		self.dlg7.warningLabel.setStyleSheet('color:red')
		self.nFeatures = 0
		self.ar_clean_up()
	...
	def poly(self):
		if not hasattr(self, 'nFeatures'):
			self.nFeatures = 0
		self.toolPoly = PolygonCreator(self.canvas, self.iface)
		for action in self.actions:
			if action.text() == "Artefact remover":
				self.toolPoly.setAction(action)
		self.canvas.setMapTool(self.toolPoly)
		self.toolPoly.finished.connect(self.load_artefact_remover)

	def store_rubberbands_for_deleting(self, rb, vrtx, pnt):
		if not hasattr(self, 'rb_collection'):
			self.rb_collection = []
		if not hasattr(self, 'point_collection'):
			self.point_collection = []
		if not hasattr(self, 'vertex_collection'):
			self.vertex_collection = []
		
		self.rb_collection.append(rb)
		self.point_collection.append(pnt)
		self.vertex_collection.append(vrtx)
	
	
	def ar_print_log(self, msg):
			# get the current time
			print_log(self.dlg7, msg)

	def ar_add_result_to_canvas(self, finished, out_file_path):
		if finished is True:
			file_name = os.path.splitext(os.path.basename(out_file_path))[0]
			rlayer = self.iface.addRasterLayer(out_file_path, file_name, "gdal")
			if rlayer:
				set_raster_symbology(rlayer)
				self.ar_print_log("The artefact were removed successfully,")
				self.ar_print_log(
					"and the resulting layer is added to the map canvas with the following name: {}.".format(file_name))
			else:
				self.ar_print_log("The algorithm has removed artefacts successfully,")
				self.ar_print_log("however the resulting layer did not load. You may need to load it manually.")
				self.ar_print_log("The modified raster is saved at: {}".format(out_file_path))
			self.finish_artefact_remover()
		else:
			self.stop_artefact_remover()
	def finish_artefact_remover(self):
			self.dlg7.cancelButton.setEnabled(False)
			self.dlg7.runButton.setEnabled(True)
			self.dlg7.warningLabel.setText('Done!')
			self.dlg7.warningLabel.setStyleSheet('color:green')
			self.ar_clean_up()
	def ar_clean_up(self):
		self.iface.actionPan().trigger()
		self.toolPoly.removePolygons(self.rb_collection, self.point_collection, self.vertex_collection)
		self.nFeatures = 0
